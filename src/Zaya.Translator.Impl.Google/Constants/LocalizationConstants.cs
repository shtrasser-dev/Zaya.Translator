namespace Zaya.Translator.Impl.Google.Constants;

internal static class LocalizationConstants
{
    public const string EngineName = "Google_EngineName";
    public const string EngineDesc = "Google_EngineDesc";
    public const string AutoDetectLanguage = "Google_AutoDetectLanguage";
    public const string AutoDetectLanguage_Desc = "Google_AutoDetectLanguage_Desc";
    public const string SourceLanguage = "Google_SourceLanguage";
    public const string SourceLanguage_Desc = "Google_SourceLanguage_Desc";
    public const string TargetLanguage = "Google_TargetLanguage";
    public const string TargetLanguage_Desc = "Google_TargetLanguage_Desc";
    public const string Err_RequestFailed = "Google_Err_RequestFailed";
    public const string Err_ParseFailed = "Google_Err_ParseFailed";
}
