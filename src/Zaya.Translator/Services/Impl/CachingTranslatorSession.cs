...
/// using an <see cref="ITranslationCache"/> instance. Both single-text and batch
/// <c>TranslateAsync</c> calls are intercepted: cached texts are returned
/// immediately, and only uncached texts are forwarded to the inner session.
/// </summary>
public sealed class CachingTranslatorSession : ITranslatorSession
{
    ...
    /// </summary>
    /// <param name="inner">The inner translator session to delegate to on cache misses.</param>
    /// <param name="cache">The cache store to use.</param>
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="inner"/> or <paramref name="cache"/> is null.</exception>
